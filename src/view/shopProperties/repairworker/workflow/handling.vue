                  ...
                  <el-col :span="12">
                    <el-dropdown trigger="click">
                      ...
                      <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item><vxe-button size="mini" content="完成订单" @click="finishOrder(row)"></vxe-button></el-dropdown-item>
                        <el-dropdown-item><vxe-button size="mini" content="申请增援" @click="applyReinforce(row)"></vxe-button></el-dropdown-item>
                      </el-dropdown-menu>
                    </el-dropdown>
                  </el-col>
        ...
        methods:{
            ...
            getData(){
                let item = localStorage.getItem("repairinfo");
                  this.$axios.get('/api/sp/repair/handling/'+this.$data.currentPage+'/'+this.$data.pageSize+"/"+JSON.parse(item).repairWorkerId)
                    .then(function (response) {
            ...
            finishOrder(row){
                this.$modal.confirm('确认操作').then(type => {
                    if( type == 'confirm'){
                        this.$axios.post('/api/sp/repair/order/finish/'+this.$data.userid,row)
                            .then(function (response) {
            ...
            applyReinforce(row){
                ...
                this.$modal.confirm('确认操作').then(type => {
                    if( type == 'confirm'){
                        this.$axios.get('/api/sp/repair/order/applayReinforce/'+row.distributeOrderId)
                            .then(function (response) {
                                if(response.data.status == '200'){
                                    let loding = this.createLoding();
                                    setTimeout(() => {
