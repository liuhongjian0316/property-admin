        ...
        <el-card style="width: 98%;margin: 0 auto;">
          <vxe-table
            ...
            <vxe-table-column field="state" title="状态">
              <template v-slot="{ row }">
                <span v-if="row.state == 0"><div class="offlinestate"></div><el-tag type="info">待接单</el-tag></span>
              </template>
            </vxe-table-column>
                  ...
                  <el-col :span="12">
                    <el-dropdown trigger="click">
                      ...
                      <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item><vxe-button size="mini" content="接单" @click="receiveOrder(row.reward_work_oder_id)"></vxe-button></el-dropdown-item>
                      </el-dropdown-menu>
                    </el-dropdown>
                  </el-col>
          ...
          <!--refuse page-->
          <vxe-modal v-model="refusePage"
                     id="13205513232328230662"
                     title="拒单" width="800" height="420"
                     resize remember
                     :fullscreen=false
                     :esc-closable=true>
            <template v-slot>
              <mavon-editor v-model="refuseForm.causeContent"
                            placeholder="请详细填写拒单说明，方便审核"
                            ref="rmd" @imgAdd="$imgAdd2" @change="change" style="max-height: 200px"/>

              <div style="float: right;margin-top: 20px">
                <el-button v-if="refuseForm.causeContent ==''" class="editor-btn" size="mini" type="primary" disabled="disabled">请填写拒单原因</el-button>
                <el-button v-if="refuseForm.causeContent !=''" class="editor-btn" size="mini" type="primary" @click="refuse">提交</el-button>
                <el-button class="editor-btn" size="mini" type="primary" @click="refusePage=false">关闭</el-button>
              </div>
            </template>
          </vxe-modal>
...
<script>
    export default {
        ...
        methods:{
            ...
            getData(){
                this.$axios.get('/api/sp/repair/grabOrder/list/'+this.$data.currentPage+'/'+this.$data.pageSize)
                    .then(function (response) {
            ...
            //接单 开始维修
            receiveOrder(id){
                this.$modal.confirm('详细查看派单详情之后，确认操作').then(type => {
                    if( type == 'confirm'){
                        this.$axios.get('/api/sp/repair/reward/order/receive/'+id+"/"+this.$data.userid)
                            .then(function (response) {
            ...
            //拒单
            refuseOrder(row){
                this.$data.refusePage = true;
                this.$data.refuseForm.worker = this.$data.userid;
                this.$data.refuseForm.workOrderId = row.workOrderId;
            },
            //拒单提交
            refuse(){
                this.$data.refuseForm.cause = this.$data.html;
                this.$axios.post('/api/sp/worker/order/refuse',
                    this.$data.refuseForm)
                    .then(function (response) {
                        if(response.data.status == '200'){
                            let loding = this.createLoding();
                            setTimeout(() => {
