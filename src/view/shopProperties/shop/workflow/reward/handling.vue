      ...
      <el-col :span="24" style="margin-top: 20px">
        <el-card style="width: 98%;margin: 0 auto;">
          <vxe-table
            ...
            <vxe-table-column field="state" title="接单状态" formatter="formatDate">
              <template v-slot="{ row }">
                <span v-if="row.state == 0"><div class="offlinestate"></div><el-tag type="info">未接单</el-tag></span>
            ...
            <vxe-table-column title="操作">
              <template v-slot="{ row }">
                <vxe-button v-if="row.state != 3 && row.state != 1" size="mini" content="修改" @click="edit(row.reward_work_oder_id)"></vxe-button>
                <vxe-button v-if="row.state != 3 && row.state != 1" size="mini" content="删除" @click="del(row.reward_work_oder_id)"></vxe-button>
                <vxe-button v-if="row.state == 3" size="mini" content="撤回" @click="recall(row.reward_work_oder_id)"></vxe-button>
              </template>
            </vxe-table-column>
          </vxe-table>
...
<script>
    export default {
        ...
        methods:{
            ...
            getData(){
                this.$axios.get('/api/sp/shop/reward/handing/'+this.$data.currentPage+'/'+this.$data.pageSize+"/"+this.$data.shopId)
                    .then(function (response) {
            ...
            save(formName){
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        this.$axios.post('/api/sp/shop/reward/order/save',
                            this.$data.form)
                            .then(function (response) {
            ...
            del(id){
                this.$modal.confirm('确认删除吗？').then(type => {
                    if( type == 'confirm'){
                        this.$axios.get('/api/sp/shop/reward/order/delete/'+id)
                            .then(function (response) {
                                if(response.data.status == '200'){
                                    let loding = this.createLoding();
                                    setTimeout(() => {
