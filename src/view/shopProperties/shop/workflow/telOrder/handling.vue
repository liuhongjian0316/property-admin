      ...
      <el-col :span="24" style="margin-top: 20px">
        <el-card style="width: 98%;margin: 0 auto;">
          <vxe-table
            ...
            export-config
            import-config>
            <vxe-table-column field="work_order_cs_id" title="编号"></vxe-table-column>
            ...
            <vxe-table-column title="操作">
              <template v-slot="{ row }">
                <vxe-button v-if="row.state != 3 && row.state != 1" size="mini" content="修改" @click="edit(row.work_order_cs_id)"></vxe-button>
                <vxe-button v-if="row.state != 3 && row.state != 1" size="mini" content="删除" @click="del(row.work_order_cs_id)"></vxe-button>
                <vxe-button v-if="row.state == 3" size="mini" content="撤回" @click="recall(row.work_order_cs_id)"></vxe-button>
              </template>
            </vxe-table-column>
          </vxe-table>
              ...
              <div style="width: 100%;margin: 0 auto;">
                <el-form  ref="form" :model="form"  :rules="rule"  label-width="80px" size="small">
                  <el-form-item label="报单类型" prop="workTypeId">
                    <el-select v-model="form.workType" filterable placeholder="请选择" style="width: 100%">
                      <el-option
...
<script>
    export default {
        ...
        methods:{
            ...
            getData(){
                this.$axios.get('/api/sp/shop/tel/handing/'+this.$data.currentPage+'/'+this.$data.pageSize)
                    .then(function (response) {
            ...
            edit(id){
                this.getWorkType();
                this.$axios.get('/api/sp/shop/telorder/'+id)
                    .then(function (response) {
            ...
            save(formName){
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        this.$axios.post('/api/sp/shop/telorder/save',
                            this.$data.form)
                            .then(function (response) {
            ...
            del(id){
                this.$modal.confirm('确认删除吗？').then(type => {
                    if( type == 'confirm'){
                        this.$axios.get('/api/sp/shop/telorder/delete/'+id)
                            .then(function (response) {
                                if(response.data.status == '200'){
                                    let loding = this.createLoding();
                                    setTimeout(() => {
            ...
            recall(id){
                this.$modal.confirm('请与维修员协商后，确认操作').then(type => {
                    if( type == 'confirm'){
                        this.$axios.get('/api/sp/shop/tel/order/recall/'+id)
                            .then(function (response) {
                                if(response.data.status == '200'){
                                    let loding = this.createLoding();
                                    setTimeout(() => {
