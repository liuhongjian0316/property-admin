      ...
      <div class="right">
        ...
        <el-dropdown trigger="click">
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item><router-link to="as">系统设置<i class="el-icon-arrow-right"></i></router-link></el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
        <!--welcome information-->
        <span style="font-size: 12px; color: grey">欢迎账号<b>{{username}}</b>访问物业管理系统</span>
        <!--消息通知-->
        <el-dropdown trigger="click">
          <span class="el-dropdown-link">
...
<script>
    export default {
        ...
        methods:{
            getData(){
                this.$axios.get('/api/sp/service/notices/type/'+1)
                    .then(function (response) {
                       this.$data.msgNum = response.data
                    }.bind(this))
            ...
            getName(){
                ...
                this.$data.name = JSON.parse(item).name
                this.$data.username = JSON.parse(item).username
            },
            exit(){
                localStorage.removeItem('Authorization');
                localStorage.removeItem('shopinfo');
                this.$router.push("/sp/shop/login");
            },
            handleFullScreen(){
