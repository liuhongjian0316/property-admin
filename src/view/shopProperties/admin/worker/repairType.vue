      ...
      <el-col :span="24" style="margin-top: 20px">
        <el-card style="width: 98%;margin: 0 auto;">
          <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item>工种信息</el-breadcrumb-item>
          </el-breadcrumb>
        </el-card>
      </el-col>
    </el-row>
          ...
          <div>
            <el-row :gutter="70">
              <el-form  ref="roleForm" :model="searchForm"  label-width="80px" size="small">
                <el-col :span="12">
                  <el-input placeholder="请输入工种类型" v-model="searchForm.type"></el-input>
                </el-col>
                <el-col :span="12">
          ...
          <el-card style="width: 98%;margin: 0 auto;">
            <vxe-toolbar :refresh="{query: reload}" import export print custom>
              <template v-slot:buttons>
                <el-button type="primary" class="el-icon-plus" size="mini" @click="addPage=true">添加工种类型</el-button>
                <!--add shop-->
                  ...
                  <template v-slot>
                    ...
                    <div style="width: 80%;margin: 0 auto;">
                      <el-form  ref="roleForm" :model="roleForm"  :rules="rules"  label-width="80px" size="small">
                        <el-form-item label="类型名称" prop="repairWorkerTypeName">
                          <el-input v-model="roleForm.repairWorkerTypeName" placeholder="请输入类型名称"> </el-input>
                        </el-form-item>
                  ...
                  <template v-slot>
                    ...
                    <div style="width: 80%;margin: 0 auto;">
                      <el-form  ref="roleForm" :model="editRoleForm"  :rules="editRules"  label-width="80px" size="small">
                        <el-form-item label="类型名称" prop="repairWorkerTypeName">
                          <el-input v-model="editRoleForm.repairWorkerTypeName" placeholder="请输入类型名称"> </el-input>
                        </el-form-item>
                        <el-form-item label="备注信息">
            ...
            <vxe-table
              ...
              export-config
              import-config>
              <vxe-table-column field="repairWorkerTypeId" title="工种类型ID"></vxe-table-column>
              <vxe-table-column field="repairWorkerTypeName" title="类型名称"></vxe-table-column>
              <vxe-table-column field="remark" title="备注信息"></vxe-table-column>
              <vxe-table-column title="操作">
                <template v-slot="{ row }">
                  <vxe-button size="mini" content="修改" @click="edit(row.repairWorkerTypeId)"></vxe-button>
                  <vxe-button size="mini" content="删除" @click="del(row.repairWorkerTypeId)"></vxe-button>
                </template>
              </vxe-table-column>
            </vxe-table>
...
<script>
    export default {
        ...
        data(){
            ...
            var checkAddName = (rule, value, callback) => {
                if (!value) {
                    ...
                    //axios 请求后端查重接口
                    this.$axios.get('/api/sp/admin/repairtype/checkAddName?name='+value) //接口
            ...
            var checkEditName = (rule, value, callback) => {
                if (!value) {
                    ...
                    //axios 请求后端查重接口
                    this.$axios.get('/api/sp/admin/repairtype/checkEditName/'+value+'/'+this.$data.editRoleForm.repairWorkerTypeId) //接口
                        .then(function (response) {
                            //根据返回判断
                            return response.data ?  callback() : callback(new Error('该类型已存在'));
                        }.bind(this))
            ...
            return{
                ...
                editRules: {
                    repairWorkerTypeName: [
                        { required: true, message: '请填写工种类型名称', trigger: 'blur' },
        ...
        methods: {
            //get worker type data
            getData(){
                this.$axios.get('/api/sp/admin/repairtype/page/'+this.$data.currentPage+'/'+this.$data.pageSize+'?type='+this.$data.searchForm.type)
                    .then(function (response) {
            ...
            handleCurrentChange(val) {
                ...
                //currentPage 变化监听
                this.$axios.get('/api/sp/admin/repairtype/page/'+val+'/'+pageSize+"?type="+this.$data.searchForm.type)
                    .then(function (response) {
                        this.$data.total = response.data.records;
                        this.$data.tableData = response.data.rows;
                    }.bind(this))
            ...
            add(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.post('/api/sp/admin/repairtype/add',
                            this.$data.roleForm)
                            .then(function (response) {
            ...
            edit(id){
                //根据id赋值
                this.$axios.get('/api/sp/admin/workertype/id/'+id).then(function (response) {
            ...
            save(formName){
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.post('/api/sp/admin/repairtype/save',
            ...
            del(id){
                this.$modal.confirm('确认删除吗？').then(type => {
                    if( type == 'confirm'){
                        this.$axios.get('/api/sp/admin/repairtype/delete/'+id)
                            .then(function (response) {
                                if(response.data.status == '200'){
                                    let loding = this.createLoding();
                                    setTimeout(() => {
